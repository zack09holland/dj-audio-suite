...
class Tracklist:
    ...
    def fetch(self):
        ...
        self.tracks = []
        track_divs = soup.find_all(
            "div", {"class": "tlpItem"}
        )  # Find div objects for tracks
        for track_div in track_divs:
            t = Track(track_div)  # TODO Get external links?
            if track_div.find(
                "i", {"title": "mashup linked position"}
            ):  # Track part of mashup -> attach to previous entry in tracklist
        ...
        self.track_nums = [
            span.text.strip()
            for span in soup.find_all("span", id=re.compile("_tracknumber_value"))
        ]
        self.cues = [
            div.text.strip() for div in soup.find_all("div", class_="cueValueField")
        ]
    ...
    def load_metadata(self, left_pane):
        ...
        try:
            self.date_recorded = (
                left_pane.find("span", title="tracklist recording date")
                .parent.parent.select("td")[1]
                .text
            )
        ...
        for interaction_detail in left_pane.find_all(
        ...
        self.genres = (
            left_pane.find("td", id="tl_music_styles").text.split(", ")
            if left_pane.find("td", id="tl_music_styles")
            else []
        )

        try:
            tracklist_DJs_location = left_pane.find_all("table", class_="sideTop")
            tracklist_DJs_location = [
                person_place.find("a") for person_place in tracklist_DJs_location
            ]
            ...
            for person_place in tracklist_DJs_location:
                ...
                if re.search("\/source\/", person_place.get("href")):
                    tracklist_source[
                        person_place.parent.parent.parent.find("td").contents[0]
                    ] = person_place.text
            ...
            AttributeError
        ):  # Couldn't find tracklist sources (e.g. DJs, festival, radio show, etc.)
            pass
...
class Artist:
    ...
    def __init__(self, span=None, name=None):
        ...
        self.artist_id = (
            self.url.split("artist/")[1].split("/")[0] if self.url else None
        )
...
class Track:
    ...
    def __init__(self, soup, fetch=False):
        ...
        # Get basic info
        self.full_title = (
            soup.find("span", {"class": "trackValue"}).text.strip().replace("\xa0", " ")
        )
        self.full_artist, self.title = tuple(self.full_title.split(" - ", maxsplit=1))

        # Get info from metadata
        meta_data = {
            meta["itemprop"]: meta["content"] for meta in soup.find_all("meta")
        }  # keys: 'name', 'byArtist', 'publisher', 'duration', 'genre', 'url'
        ...
        try:
            ...
            self.artist = Artist(
                name=(
                    meta_data["byArtist"]
                    if "byArtist" in meta_data
                    else self.full_artist
                )
            )
